...
function HomeCenter({ aPost, urlUsers, idUsers }) {
  let urlPosts = baseUrlPosts
  const [ tweetValue, setTweetvalue] = useState('')
  const [ arr, setArr] = useState([])

  useEffect(() => {
    watchPosts().then(item => setArr(item))
  }, [setArr])
  
  const handleChange = event => {
    setTweetvalue(event.currentTarget.textContent)
  }
  ...
  return (
    <>
      ...
      <form action="" onSubmit={handleSubmit}>
        <div className="Home__center_whatprofile"> 
          ...
          <div className="Home__center_whatprofile-content">
            ...
            <div className="Home__center_whatprofile-svg">
              <div className="Home__center_whatprofile-svgs">
                {/* <input name="files" id="files" multiple type="file" onChange={(e) => uploadFiles(e.target.files)} /> */}
      ...
      {
        arr.length === 0 ? 
        <div style={{color:"blue", fontSize:"50px"}}>Cargando...</div> : 
        arr.map(posts =>
