...
function HomeCenterSvgs({ tweetValue }) {
  const watchPosts = async () => {
    const res = await axios.get( baseUrlPosts )
    store.dispatch({ type: 'seePosts', payload: res.data })
  }
  ...
  return (
    <div className="Home__center_whatprofile-svg">
      ...
      <label className="home__center_contain_nextbutton" htmlFor="TweetId">
        <input className={ empty } type="submit" id="TweetId" value="Tweet" onClick={ watchPosts } />
      </label>
    </div>
  )
}

export default HomeCenterSvgs
